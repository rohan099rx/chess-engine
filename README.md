...
### UCI: show all root moves (MultiPV)

By default the engine prints a single best line.

To see evaluations for many (or all) legal root moves, set `MultiPV` high:
...
- The engine prints one `info ... multipv N ...` line per root move (up to `MultiPV`).
- Only `multipv 1` includes a full-depth `pv ...` line; the other entries include just the root move in `pv`.
