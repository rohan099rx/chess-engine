...
def get_book_entries(board, limit=8):
    """Return a list of candidate book moves for this position.

    Each entry is (engine_move, weight), where engine_move is a 3-tuple
    (start, end, promo) and weight is the Polyglot weight.

    This does not provide win/draw/loss percentages; Polyglot books only store
    weights (roughly: popularity/quality in the source book).
    """
    ...
    for entry in entries[: max(1, int(limit))]:
        out.append((_polyglot_move_to_engine_move(entry.move), int(entry.weight)))
    return out
...
def get_book_move(board):
    ...
    start, end, promo = _polyglot_move_to_engine_move(selected.move)
    return (start, end) if promo is None else (start, end, promo)
